  ...
  for (uint i=0;i<vox_num_X;i++)
   for (uint j=0; j<vox_num_Y;j++)
     ...
     if (Vox[i][j][k]!=NULL)
     {
         ...
         lp.SetAttribute(LabelTag,11);  /// OR assign label 11 to non-empty centers
     ...
     else
     {
         ...
         LaserPoint lp (X,Y,Z);
         lp.SetAttribute(LabelTag,10);  /// assign label 10 to empty centers
...
int LaserVoxel::VoxIndex(LaserPoint point)
{
    uint vox_inx_X=index_from_realX(point.GetX());
    uint vox_inx_Y=index_from_realY(point.GetY());
    uint vox_inx_Z=index_from_realZ(point.GetZ());
