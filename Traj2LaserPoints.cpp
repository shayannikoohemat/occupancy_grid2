...
LaserPoint traj2laserpoints(LaserPoint current_laserPoint, LaserPoints traj_points, vector<double> traj_points_timetag)
{
    ...
    //sort(traj_points.begin(), traj_points.end(), comp_attribute_laserpoints); //commented because is done in main function

    /// generate a vector of traj_TimeTag
/*    std::vector<double> traj_points_timetag;  //commented because is done in main function
